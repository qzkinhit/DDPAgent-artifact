#!/usr/bin/env python3
"""Generate compact DemandPrep result figures from included experiment tables."""
...
def make_framework_placeholder():
    ...
    ax.text(0.50, 0.94, "DemandPrep separates what action to take from how operators execute it", ha="center", fontsize=10.5, weight="bold")

    fig.tight_layout(pad=0.2)
    fig.savefig(FIG_DIR / "demandprep_framework_placeholder.pdf", bbox_inches="tight")
    fig.savefig(FIG_DIR / "demandprep_framework_placeholder.png", bbox_inches="tight")
    plt.close(fig)
...
def make_instance_figure():
    ...
    fig.savefig(FIG_DIR / "demandprep_instance_flow.pdf", bbox_inches="tight")
    fig.savefig(FIG_DIR / "demandprep_instance_flow.png", bbox_inches="tight")
    fig.savefig(FIG_DIR / "demandprep_instance_flow.svg", bbox_inches="tight")
    plt.close(fig)


def make_evidence_figure():
    ads = pd.read_csv(OUT_FINAL / "demandprep" / "demandprep_summary.csv")
    base = pd.read_csv(OUT_FINAL / "baseline_eval" / "artificial" / "baseline_ml_summary.csv")
    hospital_ads = OUT_HOSPITAL_TARGET / "demandprep" / "demandprep_summary.csv"
    ...
    for dataset in dataset_order:
        ...
        for _, row_data in cur.iterrows():
            ...
            demandprep_score = row_data["downstream_fixed_after"]
            wins += int(len(baselines) > 0 and demandprep_score >= baselines.max() - 1e-12)
            improves += int(demandprep_score > no_op + 1e-12)
    ...
    width = 0.24
    ax.bar([i - width for i in x], win_counts, width=width, color=COLORS["demandprep"], edgecolor=COLORS["line"], linewidth=0.55, label=r"$\geq$ best fixed")
    ...
    fig.savefig(FIG_DIR / "demandprep_selected_evidence.pdf", bbox_inches="tight")
    fig.savefig(FIG_DIR / "demandprep_selected_evidence.png", bbox_inches="tight")
    fig.savefig(FIG_DIR / "demandprep_selected_evidence.svg", bbox_inches="tight")
    plt.close(fig)
