...
def default_dataset_configs(data_root: Optional[Path] = None) -> Dict[str, DatasetConfig]:
    ...
    configs = {
        "beers": DatasetConfig(
            ...
            notes="Natural overlap between the packaged action-allocation benchmark and UniClean.",
        ),
    ...
    for path in (d.iterdir() if d.exists() else []):
        ...
        if dirty.exists():
            # These are available for legacy controller-only experiments. The
            # orchestrated DemandPrep pipeline focuses on UniClean-compatible tables.
            configs[f"demandprep_policy:{path.name}"] = DatasetConfig(
                name=f"demandprep_policy:{path.name}",
                source="demandprep_policy",
                dirty_path=dirty,
                ...
                target="",
                cleaner_profile="",
                notes="Packaged legacy action-allocation benchmark data; no UniClean profile by default.",
            )
    return configs
