...
from .result_assets import (
    ...
    find_legacy_benchmark_root,
    find_uniclean_result_root,
    result_dataset_names,
    scan_asset_catalog,
)
from .summarize import summarize_demandprep_runs


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="demandprep", description="Agentic data cleaning pipeline")
    ...
    _add_policy_runtime_args(p_run)
    ...
    _add_policy_runtime_args(p_suite)
    ...
    _add_policy_runtime_args(p_scenarios)
    p_scenarios.add_argument("--quiet", action="store_true")

    p_baseline = sub.add_parser("eval-baselines", help="Evaluate baseline cleaned CSVs with DemandPrep ML tasks")
    ...
    p_catalog.add_argument("--legacy-benchmark-root", type=Path, default=None)
    ...
    p_snapshot.add_argument("--legacy-source", type=Path, default=None)
    p_snapshot.add_argument("--destination", type=Path, default=Path("result_assets"))

    p_summary = sub.add_parser("summarize-runs", help="Merge DemandPrep metrics.json files into one CSV")
    ...
    if args.command == "list-datasets":
        ...
        for name in packaged_suite():
            cfg = configs[name]
            print(f"{name}\t{cfg.task_type}\t{cfg.target}\t{_display_path(cfg.dirty_path)}")
        return 0
    ...
    if args.command == "catalog-assets":
        df = scan_asset_catalog(args.uniclean_result_root, args.legacy_benchmark_root)
    ...
    if args.command == "prepare-snapshot":
        ...
        legacy_source = args.legacy_source or find_legacy_benchmark_root(None)
        dest = copy_result_snapshots(uniclean_source, legacy_source, args.destination)
        print(json.dumps({"destination": str(dest)}, indent=2))
        return 0

    if args.command == "summarize-runs":
        path = summarize_demandprep_runs(args.output_root)
...
def _add_policy_runtime_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--ve-source", choices=["uniclean", "policy"], default="uniclean")
