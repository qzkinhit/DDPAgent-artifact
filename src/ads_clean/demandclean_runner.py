...
def run_demandclean_plan(
    ...
    dc = DemandClean(
        ...
        save_path=str(run_dir / "demandclean"),
    ...
    inference = dc._two_phase_inference
    env = inference._env if inference is not None else None
    if env is None:
        raise RuntimeError("DemandClean did not create a two-phase environment")
    X_current, y_current, keep_mask = env.get_current_data()
    action_counts = env.get_action_counts()
    decision_log = env.get_decision_log()
    raha_used = bool(getattr(dc.detector, "raha_cost_info", {}).get("raha_total_cost", 0))
    return DemandPlanResult(
        controller=dc,
