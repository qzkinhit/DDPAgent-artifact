...
def find_demandclean_benchmark_root(explicit: Optional[Path] = None) -> Path:
    return _first_existing_root(
        explicit,
        os.environ.get("ADS_DEMANDCLEAN_BENCHMARK_ROOT"),
        RESULT_ASSET_DIR / "DemandClean-Benchmark",
        Path("/Users/qianzekai/PycharmProjects/DemandClean-Benchmark"),
        label="DemandClean-Benchmark",
    )
...
def scan_asset_catalog(uniclean_root: Optional[Path] = None, demandclean_root: Optional[Path] = None) -> pd.DataFrame:
    rows: List[Dict[str, object]] = []
    uroot = find_uniclean_result_root(uniclean_root)
    droot = find_demandclean_benchmark_root(demandclean_root)
    ...
    for path in sorted((droot / "results_and_logs" / "summary").glob("*.csv")):
        rows.append(_csv_asset_row(path, droot, "legacy_summary", "demandclean_benchmark"))
...
def copy_result_snapshots(
    ...
    _copytree_update(Path(demandclean_source), dest / "DemandClean-Benchmark")
    return dest
