...
# ---------------------------------------------------------------------------
try:
    # 作为 demandprep_policy 包的子模块运行
    from demandprep_policy.tools._compat import get_classifier, get_regressor
except ImportError:
    pass
...
def compute_model_tolerance(
    ...
    - P_dc      : 在 DemandPrepPolicy 清洗后数据上训练 → 测试性能
    ...
    Args:
        ...
        X_clean: 干净数据特征
        y_clean: 干净数据标签
        X_result: DemandPrepPolicy 清洗后的特征
        y_result: DemandPrepPolicy 清洗后的标签
        task_type: 'classification' / 'regression' / 'clustering'
...
def _plot_tolerance_bar(
    ...
    bars3 = ax1.bar(x + width, p_dc_vals, width, label='P_dc (DemandPrepPolicy)', color='#3498db')
...
def tolerance_from_dataframes(
    ...
    Args:
        ...
        result_df: DemandPrepPolicy 清洗后的 DataFrame
        label_column: 标签列名
        task_type: 任务类型
        models_list: 模型列表
        save_dir: 保存目录
...
def tolerance_from_csv(
    ...
    Args:
        ...
        result_path: DemandPrepPolicy 清洗后 CSV 路径
        label_column: 标签列名
        task_type: 任务类型
        models_list: 模型列表
        save_dir: 保存目录
...
def _self_test():
    ...
    # DemandPrepPolicy 结果: 修复了大部分缺失值 + 部分噪声
    ...
    all_results = {
        ...
        'DemandPrepPolicy': {m: {'P_dc': v['P_dc']} for m, v in tol.items()},
